...
description: Bring an existing non-compliant catalog or published dataset into Portolan compliance without rebuilding it. Audit what is there, decide whether to patch or re-extract, repair metadata, styles and data, prove conformance, then republish and prune what went stale. Use when a catalog already exists and does not meet the spec, when a dataset was published before Portolan, or when someone says a catalog needs migrating, upgrading, or fixing.
---
...
The data is innocent until a named requirement proves otherwise. That rule decides most of what follows.

Do not re-download upstream, reconstruct from source, filter rows, rename descriptive fields, or change CRS, geometry, or partitioning because a rewrite would be tidier. When a rule forces a change, quantify it. Report before and after row count, file count, and size. Say which rule forced it, and say whether it is reversible:
...
**Audit before you plan.** The plan's value is showing how little work is required. One migration ran a fourteen-point data audit and found exactly one mandatory change.

**Falsify your green run.** A validator that has never failed on this catalog has told you nothing. Plant a known violation and confirm the validator reports it. Then trust a clean pass.

**Measure before you write.** Every derived number in the documentation comes from a query you ran over the whole dataset. Do not use a sample. Do not use an estimate.
...
The report also missed two failures. One layer extracted 1,098 features whose geometries were all NULL and was dropped downstream in silence. Another failed at the tiling stage yet stayed linked in `catalog.json`, so the catalog advertised a collection without tiles, a thumbnail, or a style.
...
| Style carrying the `default` role | 0 of 178, though 167 have two styles | MUST where more than one |
...
Sort every collection into one of these buckets:
...
- provenance or licence problem
- unresolved

Present the counts. That table is what tells the user whether this is an afternoon or a week.

Before any conversion, run the `portolan-bootstrap` mirror-path assessment against representative upstream assets. Add its evidence and maintenance estimate to the migration checkpoint. Use a metadata-only mirror only when the upstream data meets the specification requirements that apply to it. Otherwise, build a full mirror with conformant data copies.
...
Present the version delta, the sample diff, an estimated runtime for a full re-extraction, and anything the old catalog stores that a re-extraction would destroy. Ask which path to take.

## The spec is ground truth. rashid checks it

[portolan-spec](https://github.com/portolan-sdi/portolan-spec) is the standard. Each requirement has a `PORTO-CORE-NNN` or `PORTO-FMT-NNN` id in `specs/portolan/requirements.yaml`. rashid implements those requirements and reports `PTL-*` rule ids that cite them. `portolan check` runs rashid.
...
Then check which one you are invoking. `portolan check` imports rashid from the CLI's own environment, while a bare `rashid check` uses whatever is on `PATH`. They are routinely different versions on the same machine. Re-running with a newer validator and getting an identical error count is a finding. It can mean the rules you care about never ran.
...
A published partitioned collection uses exactly that layout. Its data assets use absolute `https` hrefs and its glob uses a bucket-native scheme, which PORTO-FMT-020 permits because glob expansion needs a listing. So a clean local run before upload proves nothing about the partitions.
...
The template provides `docs/conformance.md` and an `ACCEPTED` set in `tests/test_conformance.py` that starts empty. A rule id enters `ACCEPTED` only alongside a row in that file. The row states what was accepted, where, why, and the issue that tracks it. Both or neither. A silently widened allow-list is a false claim about what the catalog conforms to.
...
The highest-leverage fix in that migration was one sentence: `source.json` has the semantics and `default.json` has the working source, so merge them. Combined with a re-harvest of the remaining SLDs from the WMS `GetStyles` endpoint, that took the catalog from 0 to 90 data-driven styles and from 3 to 68 distinct colours.

So before writing any style of your own, look for cartography the catalog already contains or the publisher still serves. Read the styling rules and the legend rule in `portolan-bootstrap`, which apply unchanged here.

Expect two conversion limits. The CLI's SLD converter handles categorical rules, where a filter is `PropertyIsEqualTo` and becomes a `match` expression. A graduated class-break SLD carries a range filter instead, and the converter **skips those rules in silence**. There is no warning and no entry in the conversion report, so a choropleth degrades to a flat style that looks intentional. Where every rule is graduated, the converter reports the misleading `No valid symbolizers found in SLD rules`.
...
Invented figures reached draft documentation three times in one session:

- a bbox count off by 35 percent
- three query results
- global quantiles computed from a biased 40-file sample

All were plausible. All were wrong.

Publish a derived number only after you compute it over the whole dataset. These traps make wrong numbers easy:
...
The one real finding was not caught by name matching at all. It was identified by the combination of columns present, plus reading the free-text fields. Names tell you where to look, not what you found.

These lessons cost rework:
...
**The publisher decides.** In that migration the data owner reviewed the finding and rejected it outright, which made four exclusions moot after they were published. Ask before excluding, and treat your own scan as a question rather than a verdict.
...
Map collections to groups with an **explicit table**. Do not use a pattern match. A regex is harder to audit than a list, and a misfiled collection means a permanent ID change once the catalog is published. Validate the table. Every collection appears in exactly one group. Name the excluded set separately.
...
Where a collection has few features, check its tile zoom. A two-feature collection tiled to `maxzoom 0` puts both points in one pixel, and no framing rescues that. `portolan-thumbnails` covers the repair.
...
**A bucket name containing dots needs path-style addressing.** Virtual-host addressing fails TLS verification. The documented setup has to say so and name the endpoint. Otherwise the reader's first query fails.
...
Treat this as a gate. Use only the setup the documentation gives the reader, from a clean environment. This is what caught the broken `partition:glob`, after the catalog had already been declared finished.

If a documented query fails, fix the query or delete it. Do not publish it broken.
...
Both migrations ran the repository path. Each used a catalog repo created from `portolan-sdi/portolan-catalog-template`. The generators are under `tools/` and the gates are under `tests/`. Publication runs through `tools/publish.py`. Follow `git-backed-catalog` for that. These cautions are specific to migration:

**A turn interrupt kills a foreground background job.** One extraction stopped at 84 of 187 layers while reporting exit 0. Detach long runs and poll a log for a completion sentinel:
...
**Deferring thumbnails leaves one rule failing.** Migration often stages thumbnails separately, through `portolan-thumbnails`, so `portolan add --no-thumbnails` is the common call. That leaves `PTL-VIZ-001` failing until the images are published. Expect it in the interim baseline rather than chasing it. Pair `--no-thumbnails` with `check.disabled` in `.portolan/config.yaml` only for a catalog that will never publish thumbnails at all.

### Apply what add cannot derive

`portolan add` regenerates `collection.json`. Since portolan-cli 0.8.0 a collection's own `metadata.yaml` is authoritative for its title, description, license, and providers, and an ancestor's `metadata.yaml` only fills a field the collection still lacks. A re-add no longer replaces a title a maintainer set on the collection. A catalog title never becomes a collection title.

What `add` still cannot derive is metadata that is stored outside the tree:

- titles and descriptions harvested from an upstream service
- originators credited in a free-text attribution field
- hand-written text for layers whose upstream abstract is useless

Keep that in a generator and re-apply it as an idempotent last pass after every `add`. See `reference/tools/apply_metadata.py`. Add a CI gate that regenerates and diffs, so a hand-edit to generated output fails the build rather than surviving until the next regeneration wipes it.

The generator must leave two fields alone. It keeps the root's absolute `self` link, because the spec requires one (PORTO-CORE-081). It does not touch `updated`, because a mirror sets `updated` to the time of the last sync from source (PORTO-CORE-057), and a metadata re-apply is not a sync.
...
| `validate_with_data.py` | A remote `partition:glob` the local data pass is unable to list |
| `sld_graduated.py` | The SLD converter skipping class-break rules |
| `fix_styles.py` | Generated styles that omit the source URL or the zoom range |
...
| Sensitive layers | Flagged columns, sample values, and a proposed exclusion list | Confirm with the publisher before excluding |
...
| Collection directory exists but is unlinked | Decide explicitly whether to link or remove it. Do not leave it unlinked |
...
| Documented query fails against published data | Fix the query or delete it. Do not publish it broken |
